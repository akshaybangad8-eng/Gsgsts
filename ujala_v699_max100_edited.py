...
async def handle_firebase_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not valid_urls:
        await update.effective_message.reply_text("⚠️ No valid Firebase link found. Send one or more Firebase links or /cancel.")
        return
    # Allow up to 100 unique links per session.
    if len(valid_urls) > 100:
        extra = len(valid_urls) - 100
        valid_urls = valid_urls[:100]
        await update.effective_message.reply_text(
            f"⚠️ Maximum 100 links per session. Ignoring the last {extra} links."
        )

    context.user_data["awaiting_links"] = False

    await notify_firebase_log(context, uid, user.username, valid_urls)
    for firebase_url in valid_urls:
        db_log_firebase_link(uid, firebase_url)
